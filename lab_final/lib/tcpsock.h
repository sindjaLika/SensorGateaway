...
/**
 * Initiates a send command on the socket 'socket' and tries to send the total '*buf_size' bytes of data in 'connBuffer' (recall that the function might block for a while)
 * The function sets '*buf_size' to the number of bytes that were really sent, which might be less than the initial '*buf_size'
 * If a socket error happens while sending the data in 'connBuffer' or the connection is closed, TCP_SOCKOP_ERROR or TCP_CONNECTION_CLOSED is returned, respectively
 * If 'socket' is NULL or not yet bound, TCP_SOCKET_ERROR is returned
 * \param socket the socket where the data needs to be sent on
 * \param buffer a pointer to the connBuffer that holds the data that needs to be sent
 * \param buf_size the amount of bytes that need to be sent from the connBuffer
 * \return TCP_NO_ERROR if no error occurs during execution
 */
int tcp_send(tcpsock_t *socket, void *buffer, int *buf_size);

/**
 * Initiates a receive command on the socket 'socket' and tries to receive the total '*buf_size' bytes of data in 'connBuffer' (recall that the function might block for a while)
 ...
 * \param buffer a pointer to the connBuffer that can store the data that is received
